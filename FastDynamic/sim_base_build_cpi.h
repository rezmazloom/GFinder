...
inline void select_candidate_by_edge_type_and_sim(long long parent_data_id, long long need_label, EdgeType *edge_type_query, long long need_query_id, long long check_value)
{
	...
	for (long long j = g_nodes_neighbors_start_position_data_graph[parent_data_id]; j < g_nodes_neighbors_start_position_data_graph[parent_data_id + 1]; j++)
	{
		long long onehop = g_nodes_neighbors_list_of_edge_type_data_graph[j].node_id;
		double sim = 0;

		EdgeType *onehop_edge_type = &g_nodes_neighbors_list_of_edge_type_data_graph[j];
		...
		{
			for (long long two = g_nodes_neighbors_start_position_data_graph[onehop]; two < g_nodes_neighbors_start_position_data_graph[onehop + 1]; two++)
			{
				long long twohop = g_nodes_neighbors_list_of_edge_type_data_graph[two].node_id;
				EdgeType *twohop_edge_type = &g_nodes_neighbors_list_of_edge_type_data_graph[two];
		...
		for (long long seq_index = 0; seq_index < g_level[current_level].size(); seq_index++)
		{
			...
			for (long long j = g_nodes_adj_list_start_index_query_graph[cur_node]; j < g_nodes_adj_list_start_index_query_graph[cur_node + 1]; j++)
			{
				...
				for (long long x = 0; x < neighbor_node_unit.size; x++)
				{
					...
					result = g_position_of_neighbors_have_label_data_graph[can_id][label_cur];
					...
					for (long long y = result.first; y < result.first + result.second; y++)
					{
						...
						long long temp_node = g_nodes_neighbors_list_of_edge_type_data_graph[y].node_id;
						EdgeType child_to_current = g_nodes_neighbors_list_of_edge_type_data_graph[y];
